...
def home():
	if request.method == 'POST':
		...
		try:			
			checkExists =  cursor.execute("select domainName from dbDomainDetails where domainName='"+domain+"'")
			if checkExists == 0:
...
@app.route('/inApis', methods=["GET",'POST'])
def IndividualAPI():
	domainsData=cursor.execute("select domainName from dbDomainDetails")

	
	json_indata = {'IndividualDomainsDetails':[]}
	count=0
	for domainsList in cursor.fetchall():
		...
		jsonDomain_headers=[x[0] for x in jsonDomainCursor.description]
		jdc_datas = jsonDomainCursor.fetchall()

		# print("---------------------",jsonDomain_headers)
		dlist = domainsList[0]
		json_indata['IndividualDomainsDetails'].append({dlist:[]})

		for jd in jdc_datas:
			domain = re.search("@[\w]+", jd[3]).group().replace('@','')
			if  dlist == domain:
				json_indata['IndividualDomainsDetails'][count][domain].append(dict(zip(jsonDomain_headers,jd)))
				
		# if dlist == json_indata['IndividualDomainsDetails'][dlist]:
		# 	#json_indata['IndividualDomainsDetails'][dlist].append(dict(zip(row_headers,result)))
		# 	pass
		count=count+1
	return jsonify(json_indata)

@app.route('/dropdb', methods=["GET",'POST'])
def dropDb():
	try:
		cursor.execute("select domainName from dbDomainDetails")
		for rows in cursor.fetchall():
			jsonDomainDB = pymysql.connect(
							host='localhost',
							user='root',
			...
			jsonDomainCursor.execute("DROP DATABASE IF EXISTS "+rows[0])	
		
		cursor.execute("Truncate table dbDomainDetails")
		flash('Success','success')
	except Exception as e:
		flash(str(e)+'No Database to Delete','warning')
	return render_template("home.html")
